...
- **週替わりの季節アレンジメント** — 全17種(ひまわり・チューリップ・コスモス・芍薬・ダリア・マム・カーネーション・紫陽花・水仙・ユリ・ラベンダー・梅・桜・ミモザ・紅葉・南天・ドウダンツツジ)を月ごとに3候補ずつ、週1回自動で入れ替え。花に合わせて花瓶も変わる(透明/ロゼ/スモークガラス+水入り、白磁/青磁/黒陶/テラコッタなどの陶器)。南半球では季節を6ヶ月シフト。`?obj=<id>` で任意のアレンジに固定可。
...
- **URLパラメータ** — `?lat=&lng=`(場所固定)、`?t=`(時刻シフト)、`?obj=`(sunflower/tulip/cosmos/peony/dahlia/mum/carnation/hydrangea/suisen/lily/lavender/ume/sakura/mimosa/momiji/nanten/doudan)、`?hud=1/0`、`?info=1/0`(解説カード)。
