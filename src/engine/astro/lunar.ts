...
// in longitude/latitude (validated against example 47.a in skydial, where
// this module is ported from), far tighter than moonlight rendering needs.
...
export interface MoonPosition extends MoonEphemeris {
  ...
  /** Geocentric altitude, before the parallax correction. */
...
export function moonPosition(date: Date, loc: GeoLocation): MoonPosition {
  ...
  const { azimuth, altitude: geoAlt } = equatorialToHorizontal(eq, loc.lat, st);
